...
class ConsoleOutputRotatorObserver implements RotatorObserverInterface
{
    ...
    {
        $this->output->writeln(sprintf("Fetching records %d-%d, %d found", $offset, $limit, count($records)));
    }
    ...
    {
        $this->output->writeln(sprintf("Saving records %d-%d, %d total", $offset, $limit, count($records)));
    }
}